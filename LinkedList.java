...
public class LinkedList<E> {
    ...
    /**
    * Creates a list of one null element.
    *
    */
    public LinkedList() {
    ...
    public Iterator<E> iterator(int stop) {
        ...
        int gg = size;
        for (int i = stop+1; i<gg; i++){
            temp.removeLast();
        }

        Iterator<E> itr = temp.iterator();
        int count=0;
        Object tmp = itr.next();
        while(count<stop){
            count++;
            if(itr.hasNext()){
                ...
                throw new NoSuchElementException("The iterator has only "+count+"elements.");
            }
        }
        return itr;
    }
     ...
     * @param start the index of the first element of the iteration
     * @param stop the index of the last element of the iteration
     * @param step the value used to determine how many positions to move at a time.
     * @return an iterator for this list
     */
